...
- публичную выдачу видео из локальной read-model.

Примечание:
- в текущем core-проекте модуль `public-videos` существует как серверная выдача `status=published`;
- стратегическая цель — отдельный public backend, который синхронизируется через `external`.
